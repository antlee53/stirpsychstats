...
list.of.packages <- c("learnr","shiny","tidyverse","ggplot2","remotes")
...
Be aware that the checker cannot evaluate visual information. So while submitting your answer may return a green "correct" message, it is up to you to evaluate whether your plot meets the objective. Also remember that example code that could be considered correct is given as the third hint for every question.

### Question 1

Below is the code to create a graphic using ggplot. 
...
```{r question_1-quiz}
quiz(
  question("Select all the correct statements about the code above.",
...
```{r question_2, exercise=TRUE,exercise.lines = 10}
#plot.data
...
#Note the code checker will expect that you put `2` here.
```
...
```{r question_4, exercise=TRUE,exercise.lines = 10}
#plot.data
...
You are interested in adult attachment styles in romantic relationships. From 40 participants in a relationship, you measure their attachment style to their partner. Plot the frequency of the three attachment styles in your data.frame as a bar chart and save the result as `plot.output`. The data.frame is saved as the object `attachment.data`.
...
```{r question_5, exercise=TRUE,exercise.lines = 10}
#attachment.data
...
You have conducted a study investigating infant's beliefs in fairness. From thirty 10-month-olds, you measure how long infants attend to (in seconds) a surprising event (a puppet helping another puppet that previously committed a transgression). Plot the the distribution of looking times as a histogram and save the result as `plot.output`. The data.frame is saved as the object `looking.data`.
...
```{r question_6, exercise=TRUE,exercise.lines = 10}
#looking.data
...
```{r question_6-hint-1}
The geom to create a histogram is `geom_histogram()`.
```

```{r question_6-hint-2}
Remember to specify which variable is on the x-axis within the `ggplot()` function. Histograms are a one variable graphs, so there is no variable on the y-axis.
...
plot.output
#Note, you could also use geom_density() to produce a density plot.
...
By default, ggplot will use the variable names for labels on the axes. Use the `xlab()` and `ylab()` to change the axis labels to something more informative (the histogram is the same data from Question 6 above). I would suggest changing one to "Looking Time (in Seconds)" and the other to "Proportion of Responses". Also change the title of the plot to "The Length of Time Infants Spend Looking at a Surprising Event" using `ggtitle()`.
...
```{r question_7, exercise=TRUE,exercise.lines = 10}
#looking.data
...
```{r question_7-hint-1}
You can add the `xlab()`, `ylab()` and `ggtitle()` functions to your plot using the `+` symbol.
```

```{r question_7-hint-2}
An example of an informative axis label could be "Looking Time (in Seconds)" for the x-axis.
...
This week's advanced exercises focuses on introducing additional handy functions for making visualisations in ggplot and using visualisation for data diagnostics. All questions from this week's advanced exercises uses the following dataset, where IQ scores were measured for 500 people from 5 countries:
...
In the box below, data has been collected from 500 participants measuring their IQ. Create a histogram or density plot and customise the labels using `xlab()` and `ylab()`
...
You could always copy the code you used for Question 7 of the basic exercises and change the text...
```
...
In the box below, use tidyverse functions to remove participants with an IQ score of "-99" and re-plot the distribution.
...
After reviewing the data cleaning scripts, you discover that your collaborator in Italy adjusted the IQ scores so that the standard deviation for the sample equaled 1. The standard deviation for IQ scores should be 15.

In the box below, mutate the data to fix this mistake, then re-plot the data. Note: this is a particularly tricky question!
