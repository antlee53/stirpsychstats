...
You may have noticed something doesn't look quite right about the distribution of IQ scores above. There appears to be a small number of participants with a IQ score of "-99". Some statistics courses teach that missing values should be coded as an unrealistic value (e.g., -99); however, this causes some issues if you forget to exclude them or not tell your statistics program that these are missing values!
...
To facet your graph by a single variable, add `facet_wrap()` to your plot. The first argument of this function is the name of the variable you wish to split on. The variable name is preceded by the `~` symbol. This variable should be categorical for best effect.
...
In the box below, mutate the data to fix this mistake, then replot the data.
