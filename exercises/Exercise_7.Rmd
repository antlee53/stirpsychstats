...
#Question 7
if (label == "question_a1b"){
	
	correct_anxiety.data <- data.frame(ID = 1:120,
	...
	correct_anxiety.aov <- aov(anxiety ~ drug_condition*dosage,data = anxiety.data) %>%
  summary()
 	
 	if (!exists("anxiety.data")){
 		out <- list(message = "The data.frame anxiety.data appears to be altered. Do not alter this data.frame. Click the 'Start Over' button if necessary.", correct = FALSE, location = "append")
 	} else {
 		if(!identical(anxiety.data,correct_anxiety.data)){
 			out <- list(message = "The data.frame anxiety.data appears to be altered. Do not alter this data.frame. Click the 'Start Over' button if necessary.", correct = FALSE, location = "append")
 		} else {
 			if(!exists("anxiety.aov")){
 				out <- list(message = "There is no object saved as anxiety.aov. Make sure to save the output from the ANOVA as `anxiety.aov`.", correct = FALSE, location = "append")
 			} else {
 				if(!("summary.aov" %in% class(anxiety.aov))){
 					...
 					if(!identical(anxiety.aov,correct_anxiety.aov)){
 						out <- list(message = "The object saved as `anxiety.aov` does not match the correct answer. Please double-check there are no errors in the submitted code.", correct = FALSE, location = "append")
 					}
 				}
 			}
 		}
...
#Question 8
if (label == "question_a2b"){
	
	correct_engagement.data <- data.frame(
  Subject = c("BF","DK","MS","RV"),
  Early = c(23,7,11,5),
  Middle = c(22,14,14,15),
  Late = c(31,3,11,14))
	
  correct_reshaped.data <- gather(engagement.data,key = "Time",value = "Output",Early,Middle,Late) %>%
  mutate(Subject = as.factor(Subject))
  
	correct_engagement.aov <- aov(Output ~ Time + Error(Subject/Time),data = reshaped.data) %>%
  summary()
 	
 	if (!exists("engagement.data")){
 		out <- list(message = "The data.frame engagement.data appears to be altered. Do not alter this data.frame. Click the 'Start Over' button if necessary.", correct = FALSE, location = "append")
 	} else {
 		if(!identical(engagement.data,correct_engagement.data)){
 			out <- list(message = "The data.frame engagement.data appears to be altered. Do not alter this data.frame. Click the 'Start Over' button if necessary.", correct = FALSE, location = "append")
 		} else {
 		  if(!exists("reshaped.data")){
 		    out <- list(message = "The data.frame reshaped.data appears to be altered. There is no need to alter the code that creates this data.frame. Click the 'Start Over' button if necessary.")
 		  } else {
 		    if(!identical(reshaped.data,correct_reshaped.data)){
 		      out <- list(message = "The data.frame reshaped.data appears to be altered. There is no need to alter the code that creates this data.frame. Click the 'Start Over' button if necessary.")
 		    } else {
 			if(!exists("engagement.aov")){
 				out <- list(message = "There is no object saved as engagement.aov. Make sure to save the output from the ANOVA as `engagement.aov`.", correct = FALSE, location = "append")
 			} else {
 				if(!("summary.aovlist" %in% class(engagement.aov))){
 					out <- list(message = "It doesn't appear as if the `summary()` function has been used. Make sure to use it to give you interpretable output.", correct = FALSE, location = "append")
 				} else {
 					if(!identical(engagement.aov,correct_engagement.aov)){
 						out <- list(message = "The object saved as `engagement.aov` does not match the correct answer. Please double-check there are no errors in the submitted code.", correct = FALSE, location = "append")
 					}
 				}
 			}
 		}
...
correct_performance.data <- data.frame(ID = as.character(pd[,1]),
  ...
  mutate(ID = as.factor(ID))
	...
	correct_performance.aov <- aov(performance ~ test.time*regiment + Error(ID/test.time),data = performance.data) %>%
  summary()
 	
 	if (!exists("performance.data")){
 		out <- list(message = "The data.frame performance.data appears to be altered. Do not alter this data.frame. Click the 'Start Over' button if necessary.", correct = FALSE, location = "append")
 	} else {
 		if(!identical(performance.data,correct_performance.data)){
 			out <- list(message = "The data.frame performance.data appears to be altered. Do not alter this data.frame. Click the 'Start Over' button if necessary.", correct = FALSE, location = "append")
 		} else {
 			if(!exists("performance.aov")){
 				out <- list(message = "There is no object saved as performance.aov. Make sure to save the output from the ANOVA as `anxiety.aov`.", correct = FALSE, location = "append")
 			} else {
 				if(!("summary.aovlist" %in% class(performance.aov))){
 					out <- list(message = "It doesn't appear as if the `summary()` function has been used. Make sure to use it to give you interpretable output.", correct = FALSE, location = "append")
 				} else {
 					if(!identical(performance.aov,correct_performance.aov)){
 						out <- list(message = "The object saved as `performance.aov` does not match the correct answer. Please double-check there are no errors in the submitted code.", correct = FALSE, location = "append")
 					}
 				}
 			}
 		}
...
In the code box below, there is a dataset testing whether the use of an experimental drug helps with anxiety. In a randomised controlled trial, participants are either assigned to the experimental drug condition, or a control group. Participants are also randomly assigned to one of three dosage groups (low, medium, and high). Conduct the appropriate two-way ANOVA to determine whether the use of the drug, as well as dosage levels, helps with levels of anxiety. Save the results from the `aov()` function (after using the `summary()` function) to an object called `anxiety.aov`.
...
Use the `aov()` function to conduct the two-way ANOVA. Remember the formula should include the interaction between the two IVs.
```
...
anxiety.aov <- aov(anxiety ~ drug_condition*dosage,data = anxiety.data) %>%
  summary()

anxiety.aov
```

```{r question_a1b-check}
checker()
```
...
```
 ...
 Conduct the appropriate ANOVA to test this hypothesis. Save the results from the `aov()` function (after using the `summary()` function) to an object called `engagement.aov`. Note, you may need to reshape the data. If so, the code to reshape the data has been provided.
...
engagement.aov <- aov(Output ~ Time + Error(Subject/Time),data = reshaped.data) %>%
  summary()
```

```{r question_a2b-check}
checker()
```
...
performance.aov <- aov(performance ~ test.time*regiment + Error(ID/test.time),data = performance.data) %>%
  summary()
```

```{r question_a3b-check}
checker()
```
