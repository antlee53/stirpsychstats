...
correct_marmoset.data.c <- mutate(correct_marmoset.data,
					c.patience = patience - -0.2334,
					c.age = age - 7.54)
...
if(!exists("marmoset.data")){
	...
	if(!identical(marmoset.data,correct_marmoset.data)){
		...
		if(!exists("marmoset.beta")){
			out <- list(message = "Make sure to save the standardised betas from the `lm.beta()` function as the object `marmoset.beta`.", correct = FALSE, location = "append")
		} else {
			if(!("summary.lm.beta" %in% class(marmoset.beta))){
				out <- list(message = "Make sure to save the standardised betas from the `lm.beta()` function as the object `marmoset.beta`.", correct = FALSE, location = "append")
			} else {
				if(!(.31 %in% round(marmoset.beta$coefficients[,"Standardized"],2))){
		...
		if(!exists("y.model")){
			...
			if(!exists("y2.model")){
				...
				if(!exists("m.model")){
					...
					if((class(y.model) != "lm") | (class(y2.model) != "lm") | (class(m.model) != "lm")){
						out <- list(message = "One of `y.model`, `y2.model`, or `m.model` is not the output from the `lm()` function. Ensure that this is the case.", correct = FALSE, location = "append")
					} else {
						if(!("age" %in% names(y.model$coefficients) & !("imitation" %in% names(y.model$coefficients)))){
							out <- list(message = "The object `y.model` does not match the correct answer.", correct = FALSE, location = "append")
						} else {
							if(!("age" %in% names(y2.model$coefficients) & ("imitation" %in% names(y2.model$coefficients)))){
								out <- list(message = "The object `y2.model` does not match the correct answer.", correct = FALSE, location = "append")
							} else {
								if(!("age" %in% names(m.model$coefficients))){
			...
			if(!exists("y2.model")){
				...
				if(!exists("m.model")){
					...
					if((class(y.model) != "lm") | (class(y2.model) != "lm") | (class(m.model) != "lm")){
						...
						if(!("age" %in% names(y.model$coefficients) & !("imitation" %in% names(y.model$coefficients)))){
							out <- list(message = "The object `y.model` appears to have been altered. Do not alter this object.", correct = FALSE, location = "append")
						} else {
							if(!("age" %in% names(y2.model$coefficients) & ("imitation" %in% names(y2.model$coefficients)))){
								out <- list(message = "The object `y2.model` appears to have been altered. Do not alter this object.", correct = FALSE, location = "append")
							} else {
								if(!("age" %in% names(m.model$coefficients))){
									out <- list(message = "The object `m.model` appears to have been latered. Do not alter this object.", correct = FALSE, location = "append")
								} else {
									if(!exists("med.model")){
										...
										if(class(med.model) != "mediate"){
											...
											if(!identical(med.model$model.m,m.model)){
												out <- list(message = "The model specified for the 'model.m' argument does not match the correct answer.", correct = FALSE, location = "append")
											} else {
												if(!identical(med.model$model.y,y2.model)){
													out <- list(message = "The model specified for the 'model.y' argument does not match the correct answer.", correct = FALSE, location = "append")
												} else {
													if(med.model$treat != "age"){
														out <- list(message = "The IV specified in the `mediate()` function does not match the correct answer.", correct = FALSE, location = "append")
													} else {
														if(med.model$mediator != "imitation"){
															out <- list(message = "The mediator specified in the `mediate()` function does not match the correct answer.", correct = FALSE, location = "append")
														}
													}
												}
											}
...
data2 <- mutate(data,c.var1 = var1 - 53.9625,
                c.var2 = var2 - 53.0875)
lm.model <- lm(DV ~ c.var1*c.var2,data = data2)
#Values listed above are the means for each variables. Subtracting these from the values is the same a centering them.
```
...
##Get means to center variables.
marmoset.data2 <- summarise(marmoset.data,
					patience_m = mean(assertiveness),
					age_m = mean(age))
...
plot.data_modhigh <- filter(marmoset.data,patience > -0.0043)
plot.data_modlow <- filter(marmoset.data,patience < -0.0043)

ggplot(mapping = aes(x = age, y = assertiveness)) +
  geom_smooth(data = plot.data_modhigh,method = "lm",colour = "red") +
  geom_point(data = plot.data_modhigh,colour = "red") +
  geom_smooth(data = plot.data_modlow, method = "glm",colour = "blue") +
  geom_point(data = plot.data_modlow,colour = "blue")
...
quiz(question("The correct interpretation for the analysis above is...",
     ...
     answer("Overall, older marmosets are more assertive; however, for marmosets who are low in patience, age is negatively associated with assertiveness", correct = TRUE),
...
As young children get older, they get better at a problem-solving task. You hypothesise that the association between age and performance on this problem-solving task can be explained by the child's propensity to automatically imitate the behaviour of others. You collect data from 60 children between the ages of 3 and 10 years, and measure the percentage of behaviours automatically imitated, and scores on the problem solving task (see data.frame `imitation.data`). Complete the code below to determine whether it is possible that `imitation` mediates the association between `age` and `problem_solving`.
...
quiz(question("For the `imitation.data` above, select all true statements:",
     ...
     answer("There is an association between the IV (age) and the mediator (imitation).",correct = TRUE),allow_retry = TRUE, correct = "Correct! Great, the data meets all conditions for a possible mediated relationship.",incorrect = "Incorrect. Try running the `summary()` function for the models in Question 4 to see if these relationships are significant."))
```

### Question 6

For the `imitation.data`, use the `mediate()` function to detrmine the direct and indirect effect for the hypothesised mediation.
...
```{r question_6-hint-1}
Type `help(mediate)` into the console to see what arguments the `mediate()` function accepts.
```

```{r question_6-hint-2}
As a minimum, you will need to specify four arguments to the mediate function:
The 'model.m' argument is the lm.model where the mediator is predicted by the IV.
The 'model.y' argument is the lm.model where the DV is predicted by both the IV and mediator.
The 'treat' argument specifies the name if the IV.
The 'mediator' argument specifies the name of the DV.
...
med.model <- mediate(m.model,y2.model,treat = "age",mediator = "imitation")
summary(med.model)
```
