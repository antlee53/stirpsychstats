...
Conduct an independent-samples t-test using the `coffee.data` above.
...
t.test(rt ~ coffee_condition,data = coffee.data)
```
...
t.test(risk_taking ~ hc_use,data = hc_data_women_only)
```
...
#The correct function is called aov(), not anova().
...
You're interested in the development of mental time travel across species. You develop a comparative behavioural test and collect data on 40 individuals across 5 species (data.frame saved as `time.data`). Run a one-way ANOVA to determine if there is a difference in performance on the task across species. Make sure to use the `summary()` function after the `aov()` function.
...
aov(performance ~ species,data = time.data) %>%
  summary()
