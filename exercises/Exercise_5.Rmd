 	...
 	if (!exists("time.data")){
 		...
 		if(!identical(time.data,correct_time.data)){
 			...
 			if(!exists("time.aov")){
 				...
 				if("lm" %in% class(time.aov)){
 					out <- list(message = "It doesn't appear as if the `tidy()` function has been used. Make sure to use it to give you interpretable output.", correct = FALSE, location = "append")
 				} else {
...
```{r question_1}
quiz(
  question("You want to predict an individual's emotional stability from the amount of social support and their level of perfectionism. Select the correct formula below that would be used in an analysis in R.",
...
Conduct an independent-samples t-test using the `coffee.data`. Save the result as `coffee.t.test`.
...
You're interested in the development of mental time travel across species. You develop a comparative behavioural test and collect data on 40 individuals across 5 species (data.frame saved as `time.data`. Run a One-Way ANOVA to determine if there is a difference in performance on the task across species. Save the results from the `aov()` function (after it has been tidied using the `tidy()` function) to an object called `time.aov`.
...
```{r question_7-hint-2}
Make sure you use the `tidy()` function from the broom package to make the ANOVA output interpretable.
...
```{r question_8-hint-2}
If you can't remember how to make a graph using ggplot, review material from last week's workbooks/exercises.
...
As covered in the lecture series, non-parametric tests are distribution-free tests. They are useful if your data does not meet the assumptions of the parametric test - particularly the assumption about normally distributed data.
