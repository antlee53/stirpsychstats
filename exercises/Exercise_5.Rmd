...
coffee.t.test <- t.test(rt ~ coffee_condition,data = correct_coffee.data)
```
...
```{r question_4-hint-1}
There are two ways to conduct a paired-samples t-test in R. One way requires you to reshape the data. You can use the code below:
...
coffee.t.test <- t.test(rt ~ time,data = coffee.data2_reshaped,paired = TRUE)

#Solution 2:
coffee.t.test <- t.test(coffee.data2$rt_before,coffee.data2$rt_after,paired = TRUE)
```
...
You're interested in the development of mental time travel across species. You develop a comparative behavioural test and collect data on 40 individuals across 5 species (data.frame saved as `time.data`. Run a One-Way ANOVA to determine if there is a difference in performance on the task across species. Save the results from the `aov()` function to an object called `time.aov`.
