...
This week's exercises covers structural equation modelling and model comparisons, which are quite advanced statistical techniques. Therefore, all exercises this week could be considered "Advanced". You only need to complete them if you are keen to test your R skills.
...
You're interested in measuring the social anxiety of children, so you observe the behaviour of 90 children across 5 social situation and give them a score on social anxiety for each ranging from 1 to 9. You wish to test whether a latent variable of 'social anxiety' underpins the scores across the 5 social situations. Run this model using `lavaan` functions. Save the formula as `formula` and the model as `ss_model`.
...
```{r question_5-hint-1}
Latent variables are specified using the `=~` operator.
```
...
```{r question_6-hint-2}
To get the 'completely standardised solution', you will need to specify `standardized = TRUE` within the `summary()` function.
...
```{r question_6-quiz}
quiz(
  question("The covariance between the two latent variables in the 'completely standardised solution' is closest to:",
    answer(".00"),
    answer(".24"),
    answer(".53",message = "Incorrect. Check that you have the result for the 'completely standardised solution'."),
