...
All of the above join functions have the same syntax:
...
`left_join()` is one of the more useful join functions. This function keeps all data from the first (left) table and joins all columns that match in the second (right) table. This is particularly helpful if there are cases (rows) in the right table that are not in the left (and is therefore not of interest). By carefully considering which data.frame is on the left vs. right, in almost all cases, `left_join()` will suit your needs.
...
Notice when using `left_join()`, there are no matches for "Wales" in the left data.frame, so this information is removed when using `left_join()`. Also, there are multiple matches for one country (e.g., Scotland); this function matches will join on both matches appropriately. Re-run the code above, but instead use `full_join()`, and compare the results to when you use `left_join()`.
...
Up to now, we have exclusively dealt join functions, which adds variables from one data.frame to another. A totally different type of merging data.frames together is combining rows. This is common when you have two data.frames with the same variable that include different participants. This can be achieved by using the `bind_rows()` function. This function simply accepts two arguments: the two data.frames you wish to join.
